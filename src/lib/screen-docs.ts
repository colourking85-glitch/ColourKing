...
export const MODULES: ModuleDoc[] = [
  ...
  },
  {
    ...
    screens: [
      {
        code: 'FA05',
        agentNotes: 'GET /api/invoices — returns array. Filters: search, status (draft/sent/paid/overdue/cancelled/credited). Overdue auto-calculated from due_date. PATCH /api/invoices/bulk-status { ids, status } — bulk status change with state machine validation. SAP ALV-style column picker persisted in localStorage (ck_fa05_columns).',
        userFlow: 'Browse all invoices in a SAP ALV-style configurable table. Default columns: invoice number (or "Concept"), customer, plate, created date+time, due date (red if overdue), total in EUR, and status badge. Additional columns (issued date, invoice type, language) can be toggled via the gear icon — visibility is persisted in the browser.\n\nFilter by status dropdown and search by invoice number or customer name. All columns are sortable (click header). Each row has an eye icon preview button linking to the invoice preview page.\n\nMulti-select: checkboxes on each row allow selecting multiple invoices. A bulk action bar appears when items are selected, with a status dropdown (sent, cancelled, overdue, paid). For non-draft invoices (already issued), a double confirmation is required — click Apply, then Confirm Change — to prevent accidental status changes on issued documents. Draft invoices can be bulk-changed with a single click.',
        inputs: 'Search query, status filter, column visibility (gear icon), column sort, row checkboxes, bulk status change with double confirmation for non-drafts, preview button per row.',
        outputs: 'Filtered, sorted, configurable invoice list. Bulk status updates. Preview navigation.',
        crossScreen: 'Invoices are created from approved Offers (ES10) or standalone (FA01). Preview button opens FA10 preview. Payments link to Mollie (payment gateway). Credit notes are separate invoices that reference the original. VAT amounts feed into VAT Dashboard (BW05).',
      },
      {
        code: 'FA01',
        agentNotes: 'POST /api/invoices — body: { offer_id?, customer_id, vehicle_id?, locale?, due_date?, terms?, notes?, invoice_type? (standard/deposit) }. Two creation paths: from approved offer (copies lines) or standalone (manual line entry). Due date defaults to +30 days. Locale from customer record, overridable via NL/EN/TR buttons.',
        userFlow: 'Create an invoice via two methods:\n\n1. From Offer: Select an approved offer via searchable dropdown — lines are copied automatically.\n2. Standalone: Select a customer using the searchable customer dropdown (type to filter by name). Optionally search by vehicle licence plate — selecting a plate auto-selects the customer and sets the invoice language.\n\nInvoice language: auto-set from the customer\'s locale (nl/en/tr) but can be overridden using the NL/EN/TR toggle buttons. The selected locale determines which language the invoice PDF/preview renders in.\n\nInvoice type: Standard or Deposit (voorschot).\n\nSet the due date (defaults to 30 days), payment terms text, and notes. For standalone invoices, add line items manually with kind (labour/part/material/other), description, quantity, unit price, discount, and tax code.',
        inputs: 'Creation method (offer or standalone). Searchable customer selector. Searchable vehicle plate selector. Invoice language toggle (NL/EN/TR). Invoice type. Offer (searchable, approved only). Due date, payment terms, notes. Line items for standalone.',
        outputs: 'Draft invoice with locale set. Lines copied from offer or entered manually.',
        crossScreen: 'Links to the source Offer (ES10) when created from offer. Customer locale auto-propagated. Once issued, triggers document_issued notification (SY05). Appears in Invoice List (FA05) and Document Archive (DO05).',
      },
      {
        code: 'FA10',
