...
const MODULES = [
  ...
  },
  {
    ...
    screens: [
      ...
      {
        code: 'DO21',
        agentNotes: 'GET /api/handover/[id] — returns handover note. POST /api/handover — creates from job_id (guard: job stage must be ready or delivered).',
        userFlow: 'View or print the Handover Note (Afleverbon). This document is given to the customer when collecting their repaired vehicle. It lists the work performed, parts used, before/after photos, and customer sign-off section. Generated from a Job once it reaches the "ready" or "delivered" stage.',
        inputs: 'Generated from Job (no manual input).',
        outputs: 'Numbered handover note document (print-ready A4).',
        crossScreen: 'Created from Job Detail (JB10) at ready/delivered stage. Number prefix set in Number Ranges (SY03). Stored in Document Archive (DO05).',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    screens: [
      ...
      {
        code: 'PU03',
        agentNotes: 'Separate /contact and /offerte pages. Both POST to /api/leads with different source identifiers.',
        userFlow: 'Two distinct public forms:\n- Contact (/contact): General enquiry form with name, email, phone, and message. Creates a lead with source "website".\n- Offerte (/offerte): Quote request form with vehicle details (kenteken), damage description, photo upload, and contact info. Creates a lead with source "website" tagged for quoting.\n\nBoth forms are locale-aware and create leads for staff review.',
        inputs: 'Contact: name, email, phone, message. Offerte: name, email, phone, kenteken, damage description, photos.',
        outputs: 'New lead in Leads Inbox (LD05).',
        crossScreen: 'Feeds into Leads (LD05). Staff can create Offers (ES01) from offerte-sourced leads.',
      },
    ],
  },
  {
    ...
    screens: [
      ...
      {
        code: 'SY20',
        agentNotes: 'GET /api/ai/chat — streaming chat endpoint. Requires OPENAI_API_KEY or ANTHROPIC_API_KEY in env. Context-aware: sends current screen data as context.',
        userFlow: 'AI assistant panel that can answer questions about the system and help with tasks. Open it from the "AI" button in the header. The assistant has context about the current screen and can help with data interpretation, suggest next steps, and answer how-to questions. Type your question and press Enter or click Send.',
        inputs: 'Text prompt.',
        outputs: 'AI-generated response with context from the current screen.',
        crossScreen: 'Available from any screen via the header button. Can reference data from the currently viewed record.',
      },
      {
        code: 'SY25',
